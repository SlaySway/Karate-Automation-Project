...
Feature: GetCOAConfirmation API automation tests
  ...
  Scenario Outline: Validate 200 response code for a valid request
    * def ResponseDataMap = call read('classpath:common/bookfairs/jarvis/coa_controller/COARunnerHelper.feature@getCoaConfirmRunner'){USER_ID : '<USER_NAME>', PWD : '<PASSWORD>', FAIRID : '<FAIR_ID>'}
...
Scenario Outline: Validate service response schema
    * def ResponseDataMap = call read('classpath:common/bookfairs/jarvis/coa_controller/COARunnerHelper.feature@getCoaConfirmRunner'){USER_ID : '<USER_NAME>', PWD : '<PASSWORD>', FAIRID : '<FAIR_ID>'}
    Then match ResponseDataMap.StatusCode == 200
    And json ActualResponseSchema = ResponseDataMap.ResponseString
    And json ExpectedSchema = read('classpath:common/bookfairs/jarvis/coa_controller/GetCOAConfirmation_schema.json')
