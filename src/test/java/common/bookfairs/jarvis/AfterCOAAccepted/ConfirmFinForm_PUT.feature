...
#      | azhou1@scholastic.com | password1 | current     |
  ...
  Scenario Outline: Validate invoice flow for user <USER_NAME>, fair:<RESOURCE_ID>
    * def convertNumberDecimal =
    ...
    """
    Given def mongoJson = call read("classpath:common/bookfairs/bftoolkit/MongoDBRunner.feature@FindDocumentThenDeleteField"){collection:"financials",findField:"_id",findValue:"#(RESOURCE_ID)",deleteField:"confirmation"}
    Then def confirmFinFormResponse = call read('RunnerHelper.feature@ConfirmFinancialForm')
    ...
    * def currentTimestamp =
      ...
      var sdf = new SimpleDateFormat("yyyy-MM-ddhh:mm:ss.sssz");
      var date = new java.util.Date();
      return sdf.format(date);
      }
      """
    * def expDate = currentTimestamp()
    And string formattedDate = currentDocument.confirmation.$date
    And print formattedDate
    And string NewformattedDate = formattedDate.trim().replace("T","")
    Then match NewformattedDate == expDate
