...
#      | azhou1@scholastic.com | password1 | 5694296     |
  ...
  Scenario Outline: Validate invoice flow for user <USER_NAME>, fair:<RESOURCE_ID>
    ...
    * def dollarFairLevel = if(sales>= 3500) || (sales >= 1500 && sales <= 3500 ) ? dollarFairLevel = "50" : dollarFairLevel = "40" : dollarFairLevel = "30"
    And match getFinancialFormEarningsResponse.response.earnings.scholasticDollars.earned == (getFinancialFormResponse.response.earnings.sales) * dollarFairLevel/100
    And match getFinancialFormEarningsResponse.response.earnings.scholasticDollars.due == getFinancialFormResponse.response.spending.due
    And match getFinancialFormEarningsResponse.response.earnings.scholasticDollars.balance == getFinancialFormResponse.response.earnings.scholasticDollars.earned - (Math.abs(getFinancialFormResponse.response.earnings.scholasticDollars.due))
    And match getFinancialFormEarningsResponse.response.earnings.scholasticDollars.selected == earningsDocument.fairEarnings.scholasticDollars.selected
    And match getFinancialFormEarningsResponse.response.earnings.scholasticDollars.max == getFinancialFormResponse.response.earnings.scholasticDollars.balance
    ...
    And match getFinancialFormEarningsResponse.response.cash.selected == (getFinancialFormEarningsResponse.response.earnings.scholasticDollars.balance)* 0.5
